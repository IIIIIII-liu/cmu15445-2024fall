...
//  *
//  * With `ReadPageGuard`s, there can be multiple threads that share read access to a page's data. However, the
//  existence
...
//    * this default constructor is to enable placing an "uninitialized" guard on the stack that we can later move
//    assign
//    * via `=`.
//    *
...
//  private:
//   /** @brief Only the buffer pool manager is allowed to construct a valid `ReadPageGuard.` */
//   explicit ReadPageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame, std::shared_ptr<LRUKReplacer>
//   replacer,
...
//    *
//    * If we did not maintain this flag, then the move constructor / move assignment operators could attempt to
//    destruct
...
//  *
//  * With a `WritePageGuard`, there can be only be 1 thread that has exclusive ownership over the page's data. This
//  means
//  * that the owner of the `WritePageGuard` can mutate the page's data as much as they want. However, the existence of
//  a
...
//  private:
//   /** @brief Only the buffer pool manager is allowed to construct a valid `WritePageGuard.` */
//   explicit WritePageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame, std::shared_ptr<LRUKReplacer>
//   replacer,
//                           std::shared_ptr<std::mutex> bpm_latch, std::shared_ptr<DiskScheduler> disk_scheduler);
...
//    * @brief A shared pointer to the buffer pool's replacer.
//    *
//    * Since the buffer pool cannot know when this `WritePageGuard` gets destructed, we maintain a pointer to the
//    buffer
//    * pool's replacer in order to set the frame as evictable on destruction.
//    */
//   std::shared_ptr<LRUKReplacer> replacer_;
...
/**
 * @brief Base class for page guards providing common functionality.
 */
class PageGuard {
 protected:
  /** @brief The page ID of the page we are guarding. */
  ...
  /**
   * @brief The validity flag for this page guard.
   */
  bool is_valid_{false};

  /**
   * @brief Acquire the appropriate lock for the guard type.
   */
  virtual void AcquireLock() = 0;

  /**
   * @brief Release the appropriate lock for the guard type.
   */
 ...
 protected:
  /** @brief Common constructor for page guards. */
