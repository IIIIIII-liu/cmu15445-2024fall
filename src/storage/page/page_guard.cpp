...
//  * ### Implementation
//  *
//  * Make sure you don't double free! Also, think **very** **VERY** carefully about what resources you own and the
//  order
...
auto PageGuard::GetPageId() const -> page_id_t {
  BUSTUB_ENSURE(is_valid_, "tried to use an invalid page guard");
  return page_id_;
}
...
void ReadPageGuard::AcquireLock() { frame_->rwlatch_.lock_shared(); }

void ReadPageGuard::ReleaseLock() { frame_->rwlatch_.unlock_shared(); }
...
void WritePageGuard::AcquireLock() { frame_->rwlatch_.lock(); }

void WritePageGuard::ReleaseLock() { frame_->rwlatch_.unlock(); }
